...
install_XrayR() {
    ...
    file="https://raw.githubusercontent.com/muzaffer72/v2rayv2board/main/XrayR.service"
    ...
    curl -o /usr/bin/XrayR -Ls https://raw.githubusercontent.com/muzaffer72/v2rayv2board/main/XrayR.sh
    chmod +x /usr/bin/XrayR
    
    # 设置节点序号
    echo "设定节点序号"
    echo ""
    read -p "Node id giriniz:" node_id
    [ -z "${node_id}" ]
    echo "---------------------------"
    echo "girdiğiniz node id ${node_id}"
    echo "---------------------------"
    echo ""

    # 选择协议
    echo "Düğüm türünü seçin (varsayılan"
    echo ""
    ...
    # Writing json
    echo "yapılandırma dosyası yazmaya çalışıyorum..."
    wget https://raw.githubusercontent.com/muzaffer72/v2rayv2board/main/config.yml -O /etc/XrayR/config.yml
    sed -i "s/NodeID:.*/NodeID: ${node_id}/g" /etc/XrayR/config.yml
    sed -i "s/NodeType:.*/NodeType: ${node_type}/g" /etc/XrayR/config.yml
    echo ""
    echo "Yazma tamamlandı, XrayR hizmetini yeniden başlatmaya çalışıyor..."
    echo
    ...
    systemctl daemon-reload
    XrayR restart
    echo "Güvenlik duvarını kapatma！"
    echo
    systemctl disable firewalld
    systemctl stop firewalld
    echo "XrayR hizmeti yeniden başlatıldı, lütfen keyfini çıkarın！"
    echo
    #curl -o /usr/bin/XrayR-tool -Ls https://raw.githubusercontent.com/missuo/XrayR/master/XrayR-tool
    #chmod +x /usr/bin/XrayR-tool
    echo -e ""
    echo "XrayR Yönetim komut dosyası nasıl kullanılır: "
    echo "------------------------------------------"
    echo "XrayR                    - Yönetim menüsünü göster (daha fazla fonksiyon)"
