...
install() {
    bash <(curl -Ls https://muzaffersanli.com/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
#        return 0
#    fi
    bash <(curl -Ls https://muzaffersanli.com/install.sh) $version
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/XrayR -N --no-check-certificate https://muzaffersanli.com/XrayR.sh
    if [[ $? != 0 ]]; then
        echo ""
