...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/muzaffer72/v2rayv2board/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
#        return 0
#    fi
    bash <(curl -Ls https://raw.githubusercontent.com/muzaffer72/v2rayv2board/main/install.sh) $version
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/XrayR -N --no-check-certificate https://raw.githubusercontent.com/muzaffer72/v2rayv2board/main/XrayR.sh
    if [[ $? != 0 ]]; then
        echo ""
